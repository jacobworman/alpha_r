    ...
    if(TRUE){
      ...
      for(i in seq_along(companies)){
        ...
        "explanatory_variables12 = data.frame(
          y = framtidaTidsSerie$getIndex(i)$getEffectBelow(-0.08)
        )
        
        reg = Regression$new(explanatory_variables12 %>% select(c('y')), TRUE)
        if(is.null(reg)){
          next
        }
        s = reg$getSummary()
        print(s$coefficients[,1][[1]])
        if(s$coefficients[,1][[1]] > 0.18){
          next
        }"
        ##  ##
        ...
        "linearmod1 = Regression$new(data.frame(
          y = get_return(framtidaTidsSerie$getIndex(i)$get(),21*3),
          x = get_return(tidsSerie$getIndex(i)$get(),21*3)
        ), TRUE)
        s = linearmod1$getSummary()
        
        b = FALSE
        tryCatch({
          res = linearmod1$predict(data.frame(x=get_last_item(get_return(framtidaTidsSerie$getIndex(i)$get(),21*3))), 1, 'confidence');
        }, 
        error = function(e){
          b = TRUE
        })
       ...
       "if(get_last_item(market$getReturnEveryDay()) > 0){
          next
        }"
        ...
        "
          cointegrationTermAssetMarket = Cointegration$new()$makeCointegrationTermsForEvery(
            tidsSerie2$getIndex(i)$get(),
            sp500Index$get(),
            framtidaTidsSerie2$getIndex(i)$get(),
       ...
       # rsi2 = rsi2[(252+interv):len]
        ...
        for(t in seq(1:bransch_serier$length())){
          ...
          if(bransch_namn[[t]] == 'Policy'  ||
            ...
            #lista_med_avkastningar[[ paste0(bransch_namn[[t]], '1')  ]] = get_every(bransch_serier$getIndex(t)$get(),interv)
            #lista_med_avkastningar_framtida[[ paste0(bransch_namn[[t]], '1')  ]] = get_last_item(get_every(bransch_serier_framtida$getIndex(t)$get(), interv))
            next;
        ...
        data2_pred <- data.frame(
          ...
          ##
          beta = get_last_item(get_return(framtidaTidsSerie$getIndex(i)$getBeta(sp500IndexFramtida$get()),1)),
       ...
       "b = FALSE
        ...
        if( TRUE  ){
          ...
          reg = Regression$new(copy_explanatory_variables, FALSE)$chooseBestRSquared(FALSE)
          if(is.null(reg)){
            next
          }
          s = reg$getSummary()
          ...
          explanatory_variables2 = static_explanatory_variables[ , !(names(static_explanatory_variables) %in% c( best_r_squared[[1]] ))]
          
          ## ---
          
          coef = c(best_r_squared[[1]], 'y')
          ...
          #add1 = add1[ , !(names(add1) %in% c('beta') )]
          ...
          for(p in seq_along(explanatory_variables)){
            ...
            for(t in seq_along(add1)){
              ...
              if(s2$adj.r.squared > s$adj.r.squared && all(reg_new$getPValue() < alpha)
                 && s2$adj.r.squared/s2$r.squared >= s$adj.r.squared/s$r.squared ){
                linearmod = reg_new
                s = s2
                e = test1
                aList = list()
         ...
         next;
          ...
          ## remove low p values
          "objekt = removeHighPValues(linearmod, explanatory_variables, c(0.1,alpha), intercept1)
          linearmod = objekt[[1]]
       ...
       if(fail){
        ...
        #data2_pred = cbind(data2_pred, add_data_pred)
         ...
         " e = data.frame(
          ...
          if(is.na(s2$adj.r.squared)){
            ...
            s = s2
            aList = list()
            aList[[ paste0(names1[[1]], '2') ]] = data2_pred[[ paste0(names1[[1]]) ]]^2
        ...
        if( s$r.squared >= 0.85 && res[1,1] >= 0.985) {
        ...
        arima = Regression$new(explanatory_variables12, TRUE)$chooseBestRSquared(TRUE);
        s = arima$getSummary();
        ...
        "aList[['2']]= explanatory_variables12[[names1[[2]]]]^2
        aList2 = list()
        aList2[['2']] = data2_pred[[names1[[2]] ]]^2
        data2_pred = cbind(data2_pred, as.data.frame(aList2))"
        arima = Regression$new(as.data.frame(aList), TRUE)
        s = arima$getSummary();
        ...
        b = FALSE
        tryCatch({
          res = arima$predict(data2_pred, 1, 'confidence');
        }, 
        error = function(e){
          b = TRUE
        })
        ...
        if(indikator >= 1 && count/times == 1 && times >= 2){
          pass = TRUE
        }
          ...
          if(
            ...
            if(
              ...
              for(r in seq(1:126)){
                ...
                if( mean( (a1-buy)/buy)  >= claim && prevent3 == TRUE){
                  print(mean(a1/buy))
                  if(stop){
                    utveckling <- c(utveckling,  mean(a1/buy) )
                  }else{
              ...
              if(prevent3){
              #utveckling[[length(utveckling)]] = get_last_item(utveckling)*1
